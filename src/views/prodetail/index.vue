<template>
  <div class="prodetail">
    ...
    <div class="footer">
      ...
      <div class="btn-buy" @click="byFn">立刻购买</div>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      commentList: [], // 评价
      defaultImg, // 默认头像
      showPannel: false, // 弹层值
      mode: 'cart', // 标记弹层状态
      addCount: 1, // 加入购物车或购买数量
      cartTotal: 0 // 购物车已有商品数
    }
  },
  computed: {
    goodsId () {
  ...
  created () {
    this.getDetail() // 获取商品描述
    this.getComments() // 获取评价
  },
  methods: {
    onChange (index) {
      this.current = index
    },
    async getDetail () { // 获取商品数据
      const { data: { detail } } = await getProDetail(this.goodsId)
      this.detail = detail
      this.images = detail.goods_images
    },
    async getComments () { // 获取评价
