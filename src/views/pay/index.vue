<template>
  <div class="pay">
    ...
    <div class="address">
      ...
      <div class="info" v-if="this.defaultAddress" @click="$router.push('/address')">
        <div class="info-content">
          <span class="name">{{ this.defaultAddress.name }}</span>
          <span class="mobile">{{ this.defaultAddress.phone }}</span>
        </div>
    ...
    <div class="pay-list" v-if="order.goodsList">
      ...
      <div class="pay-detail">
        ...
        <div class="pay-cell">
          <span>配送费用：</span>
          <span v-if="!this.defaultAddress">请先选择配送地址</span>
...
<script>
import { getDetailAddress, getDefaultAddress } from '@/api/address'
...
export default {
  ...
  methods: {
    ...
    async getDefaultAddressFn () { // 获取默认收货地址ID
      const { data: { defaultId } } = await getDefaultAddress()
      const { data: { detail } } = await getDetailAddress(defaultId)
      this.defaultAddress = detail
      // 返回详细地址
      this.longAddress = detail.region.province + detail.region.city + detail.region.region + detail.detail
    },
    async getOrderList () {
      // 购物车结算
      if (this.mode === 'cart') {
