...
export default {
  ...
  data () {
    return {
      search: '', // 搜素框的内容
      history: getHistoryList() // 获取本地的搜素记录
    }
  },
  methods: {
    goSearch (key) { // 搜素
      ...
      if (index !== -1) { // 存在相同的项
        this.history.splice(index, 1) // 删除历史关键字
      }
      this.history.unshift(key) // 追加历史关键字

      setHistoryList(this.history) // 把搜素记录保存到本地

      this.$router.push(`/searchlist?search=${key}`) // 跳转到搜索列
    },
    clear () { // 清空搜素历史
      this.history = []
      setHistoryList([])
